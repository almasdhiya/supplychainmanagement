...
$pdf->Cell(260,10,'Rekap Proses 7',0,1,'C');
...
$data = mysqli_query($conn, "SELECT proses7.*, customer.nickname, part.nama_part, part.kode_part FROM proses7 inner join
customer on customer.id_cust = proses7.id_cust 
inner join part on part.id_part = proses7.id_part WHERE proses7.tgl = '$bulan'");
while ($row = mysqli_fetch_array($data)) {
  ...
  $cellWidth = 15;
  while ($pdf->GetStringWidth($row['qty_outtttttt']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };
  $pdf->Cell($cellWidth, 5, $row['qty_outtttttt'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);

  $cellWidth = 30;
  while ($pdf->GetStringWidth($row['qty_nggggggg']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };
  $pdf->Cell($cellWidth, 5, $row['qty_nggggggg'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);
...
ob_end_clean();
$pdf->Output('Proses 7-' . $_POST['bulan'] . '.pdf', 'I');
