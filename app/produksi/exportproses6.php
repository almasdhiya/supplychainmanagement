...
$pdf->Cell(260,10,'Rekap Proses 6',0,1,'C');
...
$data = mysqli_query($conn, "SELECT proses6.*, customer.nickname, part.nama_part, part.kode_part FROM proses6 inner join
customer on customer.id_cust = proses6.id_cust 
inner join part on part.id_part = proses6.id_part WHERE proses6.tgl = '$bulan'");
while ($row = mysqli_fetch_array($data)) {
  ...
  $cellWidth = 15;
  while ($pdf->GetStringWidth($row['qty_outttttt']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };
  $pdf->Cell($cellWidth, 5, $row['qty_outttttt'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);

  $cellWidth = 30;
  while ($pdf->GetStringWidth($row['qty_ngggggg']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };
  $pdf->Cell($cellWidth, 5, $row['qty_ngggggg'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);
...
ob_end_clean();
$pdf->Output('Proses 6-' . $_POST['bulan'] . '.pdf', 'I');
