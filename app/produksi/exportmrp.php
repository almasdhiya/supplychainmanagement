...
while ($row = mysqli_fetch_assoc($data)) {
  ...
  if ($row['sisi'] > 0) {
    $cellWidth = 15;
    while ($pdf->GetStringWidth (number_format(($row['qty_order'] * $row['sisi']))) > $cellWidth) {
      $pdf->setFontSize($tempFontSize -= 0.1);
    };

    $pdf->Cell($cellWidth, 5, number_format(($row['qty_order']* $row['sisi'])), 1, 0, "C");

    $tempFontSize = $fontSize;
    $pdf->SetFontSize($fontSize);
  } else {
    $cellWidth = 15;
    while ($pdf->GetStringWidth($row['qty_order']) > $cellWidth) {
      $pdf->setFontSize($tempFontSize -= 0.1);
    };

    $pdf->Cell($cellWidth, 5, $row['qty_order'], 1, 0, "C");
  ...
  if ($row['sisi'] > 0) {
    $cellWidth = 12;
    while ($pdf->GetStringWidth(number_format(($row['qty_order'] * $row['sisi']) - $row['stok'])) > $cellWidth) {
      $pdf->setFontSize($tempFontSize -= 0.1);
    };

    $pdf->Cell($cellWidth, 5, (number_format(($row['qty_order'] - $row['stok']))), 1, 0, "C");

    $tempFontSize = $fontSize;
    $pdf->SetFontSize($fontSize);
  } else {
    $cellWidth = 12;
    while ($pdf->GetStringWidth(number_format(($row['qty_order'] - $row['stok']))) > $cellWidth) {
      $pdf->setFontSize($tempFontSize -= 0.1);
    };
  ...
  // if ($row['sisi'] > 0) {
  //   $cellWidth = 9;
  //   while ($pdf->GetStringWidth(number_format(((($row['qty_order'] * $row['sisi']) * $row['kg_pcs']) + (0.03 * $row['qty_order'] * $row['sisi'] * $row['kg_pcs'] - $row['stok'])), 0)) > $cellWidth) {
  //     $pdf->setFontSize($tempFontSize -= 0.1);
  //   };

  //   $pdf->Cell($cellWidth, 5, number_format(((($row['qty_order'] * $row['sisi']) * $row['kg_pcs']) + (0.03 * $row['qty_order'] * $row['sisi'] * $row['kg_pcs'] - $row['stok'])), 0), 1, 0, "C");

  //   $tempFontSize = $fontSize;
  //   $pdf->SetFontSize($fontSize);
  // } else {
  //   $cellWidth = 9;
  //   while ($pdf->GetStringWidth(number_format(($row['qty_order'] * $row['kg_pcs'] + (0.03 * $row['qty_order'] * $row['kg_pcs'] - $row['stok'])), 0)) > $cellWidth) {
  //     $pdf->setFontSize($tempFontSize -= 0.1);
  //   };

  //   $pdf->Cell($cellWidth, 5, number_format(($row['qty_order'] * $row['kg_pcs'] + (0.03 * $row['qty_order'] * $row['kg_pcs'] - $row['stok'])), 0), 1, 0, "C");
  ...
  if ($row['sisi'] > 0) {
    if ($row['qty_order'] > 0 && $row['pcs_sheet'] > 0) {
      $cellWidth = 12;
      while ($pdf->GetStringWidth((number_format(($row['qty_order'] * $row['sisi'] / $row['pcs_sheet']) - $row['stok'])), 0) > $cellWidth) {
        $pdf->setFontSize($tempFontSize -= 0.1);
      };

      $pdf->Cell($cellWidth, 5, number_format(($row['qty_order'] * $row['sisi'] / $row['pcs_sheet']) - $row['stok']), 1, 0, "C");
    ...
    if ($row['qty_order'] > 0 && $row['pcs_sheet'] > 0) {
      $cellWidth = 12;
      while ($pdf->GetStringWidth((number_format(($row['qty_order'] / $row['pcs_sheet']) - $row['stok'])), 0) > $cellWidth) {
        $pdf->setFontSize($tempFontSize -= 0.1);
      };

      $pdf->Cell($cellWidth, 5, number_format(($row['qty_order'] / $row['pcs_sheet']) - $row['stok']), 1, 0, "C");
  ...
  if ($row['sisi'] > 0) {
    if ($row['qty_order'] > 0 && $row['pcs_lembar'] > 0) {
      $cellWidth = 12;
      while ($pdf->GetStringWidth((number_format(($row['qty_order'] * $row['sisi']) - $row['stok'])), 0) > $cellWidth) {
        $pdf->setFontSize($tempFontSize -= 0.1);
      };

      $pdf->Cell($cellWidth, 5, number_format(($row['qty_order'] * $row['sisi']) - $row['stok']), 1, 0, "C");
    ...
    if ($row['qty_order'] > 0 && $row['pcs_lembar'] > 0) {
      $cellWidth = 12;
      while ($pdf->GetStringWidth((number_format($row['qty_order'] - $row['stok'])), 0) > $cellWidth) {
        $pdf->setFontSize($tempFontSize -= 0.1);
      };
