...
$pdf->Cell(260, 10, 'Rekap Proses 2', 0, 1, 'C');
...
$data = mysqli_query($conn, "SELECT proses2.*, customer.nickname, part.nama_part, part.kode_part FROM proses2 inner join
customer on customer.id_cust = proses2.id_cust
inner join part on part.id_part = proses2.id_part WHERE proses2.tgl = '$bulan'");
while ($row = mysqli_fetch_array($data)) {
  ...
  $cellWidth = 15;
  while ($pdf->GetStringWidth($row['qty_outt']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };

  $pdf->Cell($cellWidth, 5, $row['qty_outt'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);

  $cellWidth = 30;
  while ($pdf->GetStringWidth($row['qty_ngg']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };

  $pdf->Cell($cellWidth, 5, $row['qty_ngg'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);
...
ob_end_clean();
$pdf->Output('Proses 2-' . $_POST['bulan'] . '.pdf', 'I');
