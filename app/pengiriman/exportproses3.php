...
$pdf->Cell(260, 10, 'Rekap Proses 3', 0, 2, 'C');
...
$data = mysqli_query($conn, "SELECT proses3.*, customer.nickname,part.nama_part, part.id_part FROM proses3 inner join
customer on customer.id_cust = proses3.id_cust 
inner join part on part.id_part = proses3.id_part WHERE proses3.tgl = '$bulan'");
while ($row = mysqli_fetch_array($data)) {
  ...
  $pdf->SetFontSize($fontSize);
   ...
   $cellWidth = 15;
  while ($pdf->GetStringWidth($row['qty_outtt']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };
  $pdf->Cell($cellWidth, 5, $row['qty_outtt'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);

  $cellWidth = 30;
  while ($pdf->GetStringWidth($row['qty_nggg']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };
  $pdf->Cell($cellWidth, 5, $row['qty_nggg'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);
...
ob_end_clean();
$pdf->Output('Proses 3-' . $_POST['bulan'] . '.pdf', 'I');
