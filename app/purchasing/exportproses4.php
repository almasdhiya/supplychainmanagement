...
$pdf->Cell(260, 10, 'Rekap Proses 4', 0, 2, 'C');
...
$data = mysqli_query($conn, "SELECT proses4.*, customer.nickname, part.nama_part, partt.kode_part FROM proses4 inner join
customer on customer.id_cust = proses4.id_cust 
inner join part on part.id_part = proses4.id_part WHERE proses4.tgl = '$bulan'");
while ($row = mysqli_fetch_array($data)) {
  ...
  $cellWidth = 15;
  while ($pdf->GetStringWidth($row['qty_outttt']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };

  $pdf->Cell($cellWidth, 5, $row['qty_outttt'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);

  $cellWidth = 30;
  while ($pdf->GetStringWidth($row['qty_ngggg']) > $cellWidth) {
    $pdf->setFontSize($tempFontSize -= 0.1);
  };

  $pdf->Cell($cellWidth, 5, $row['qty_ngggg'], 1, 0, "C");
  $tempFontSize = $fontSize;
  $pdf->SetFontSize($fontSize);
...
ob_end_clean();
$pdf->Output('Proses 4-' . $_POST['bulan'] . '.pdf', 'I');
